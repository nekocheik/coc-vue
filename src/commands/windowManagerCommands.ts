...
export function registerWindowManagerCommands(context: ExtensionContext) {
  ...
  context.subscriptions.push(
    commands.registerCommand('windowManager.forceMount', async () => {
      try {
        // Log for debugging
        console.log('[WindowManager] Starting reactive template mount');
        window.showInformationMessage('Mounting components using reactive template system...');
        ...
        // Step 2: Use the template integration system to render components
        // Import the renderAppTemplate function
        const { renderAppTemplate } = require('../../template/templateIntegration');
        
        // Create buffer router and window manager instances for template rendering
        const bufferRouter = new BufferRouter(context);
        const windowManager = new WindowManager(bufferRouter);
        
        // Call renderAppTemplate to handle the actual component rendering
        const success = await renderAppTemplate(windowManager, bufferRouter);
        
        if (success) {
          window.showInformationMessage('Reactive template mount completed successfully');
        } else {
          window.showWarningMessage('Reactive template mount completed with warnings');
        }
        
        return success;
      } catch (error) {
        window.showErrorMessage(`Error during reactive template mount: ${error}`);
        console.error('[WindowManager] Error during reactive template mount:', error);
        return false;
      }
    }),
