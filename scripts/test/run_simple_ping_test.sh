#!/bin/bash
# scripts/run_simple_ping_test.sh
# Script to start a standalone Lua ping server and run the ping test
...
echo -e "${YELLOW}Starting standalone ping server...${NC}"

# Start the simple ping server in the background
lua "$PROJECT_ROOT/scripts/simple_ping_server.lua" > ping_server.log 2>&1 &
SERVER_PID=$!
