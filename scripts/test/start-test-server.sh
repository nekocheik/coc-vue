...
# Utilise le système de ports dynamiques
...
node "$PROJECT_ROOT/test/scripts/mock-server.js" > /tmp/mock-server.log 2>&1 &
SERVER_PID=$!
...
# Attendre jusqu'à 10 secondes que le serveur soit prêt
echo -e "${YELLOW}Vérification de la disponibilité du serveur...${NC}"
for i in {1..10}; do
  if check_server; then
    echo -e "${GREEN}Serveur mock démarré et accessible sur le port 9999.${NC}"
  ...
  if [ $i -eq 10 ]; then
    echo -e "${RED}ERREUR: Impossible de se connecter au serveur mock après 10 secondes.${NC}"
